...
function EventArchiveCard({ event }: EventArchiveCardProps) {
  ...
  return (
    <motion.div
      ...
      className="group flex h-full flex-col overflow-hidden rounded-none border-2 border-gray-700 bg-gray-800 shadow-lg transition-all duration-300 hover:border-tertiary-400 hover:shadow-primary-500/30"
    >
      <Link href={`/events/${event.id}`} className="flex h-full flex-col">
        <div className="relative aspect-[3/2] w-full overflow-hidden">
          ...
          {/*  Date Badge */}
          {shortMonth && day && (
            <div className="absolute left-0 top-4 bg-tertiary-400 px-3 py-1 text-black shadow-md">
          ...
          {/* Genre Tag */}
          {event.genre?.name && (
            <div className="absolute bottom-2 right-2 bg-primary-500 px-2 py-1 text-xs font-bold uppercase tracking-wider text-white">
              {event.genre.name}
            </div>
          )}
        </div>
        ...
        <div className="flex flex-grow flex-col p-4">
          <h3 className="mb-1 line-clamp-2 text-xl font-black uppercase text-tertiary-400 transition-colors duration-300 group-hover:text-primary-500">
            {event.title}
          </h3>
          ...
          <div className="mb-3 mt-auto space-y-1 text-sm text-gray-400">
            ...
            <div className="flex items-center">
              <CalendarDays className="mr-2 h-4 w-4 flex-shrink-0 text-primary-500" />
          ...
          <div className="mt-3 border-t-2 border-dashed border-gray-700 pt-3">
            <div className="flex items-center justify-between">
              <span className="text-lg font-bold text-tertiary-400">
                {lowestPrice > 0
                  ? `From ${formatCurrency(lowestPrice, event.currency || 'USD')}`
                  : 'INFO SOON'}
              </span>
              <span className="rounded-sm border-2 border-current px-3 py-1 text-xs font-bold uppercase text-tertiary-400 transition-colors group-hover:bg-tertiary-400 group-hover:text-black">
                View Event
              </span>
            </div>
          </div>
  ...
  if (isLoading) {
    return (
      <div className="flex min-h-[calc(100vh-200px)] flex-col items-center justify-center bg-black text-white p-8">
        <Loader2 className="h-16 w-16 animate-spin text-tertiary-400" />
    ...
    return (
      <div className="flex min-h-[calc(100vh-200px)] flex-col items-center justify-center bg-black p-8 text-center text-white">
        ...
        <Button
            ...
            className="mt-8 bg-tertiary-400 px-8 py-3 text-lg font-bold text-black transition-transform hover:scale-105 hover:bg-tertiary-300"
        >
            RELOAD THE SETLIST
        </Button>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-100 px-4 py-24 sm:px-6 lg:px-8">
      ...
      <header className="relative mb-16 text-center md:mb-24">
        <div className="absolute -left-4 -top-4 -z-0 h-20 w-20 animate-pulse text-primary-900/50 opacity-30 md:h-32 md:w-32">
             <GripVertical size="100%" />
        </div>
        <div className="absolute -right-4 -bottom-8 -z-0 h-16 w-16 animate-pulse text-tertiary-900/50 opacity-30 md:h-24 md:w-24">
             <RadioTower size="100%" />
        </div>
        <h1 className="text-5xl font-black uppercase tracking-tighter text-white sm:text-6xl md:text-8xl">
          <span className="text-shadow-hard block text-tertiary-400">Event</span>
          <span className="text-shadow-hard-primary -mt-2 block text-primary-500 sm:-mt-3 md:-mt-5">Archive</span>
        </h1>
        ...
        <section id="browse-by-genre" className="mb-16 md:mb-24">
          ...
          {sortedActiveGenreNames.map((genreName) => (
            <div key={genreName} className="mb-12">
              <h3 className="mb-6 border-b-4 border-double border-primary-500 pb-2 text-3xl font-bold uppercase tracking-tight text-primary-500 md:mb-8 md:text-4xl">
                {genreName}
              </h3>
        ...
        <section id="browse-by-country">
          ...
          {sortedActiveCountryNames.map((countryName) => (
            <div key={countryName} className="mb-12">
              <h3 className="mb-6 border-b-4 border-double border-tertiary-400 pb-2 text-3xl font-bold uppercase tracking-tight text-tertiary-400 md:mb-8 md:text-4xl">
                {countryName}
              </h3>
              <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 md:gap-8">
      ...
      <style jsx global>{`
        ...
        .text-shadow-hard-primary {
           text-shadow: 2px 2px 0px #4a044e, -2px -2px 0px #4a044e, 2px -2px 0px #4a044e, -2px 2px 0px #4a044e; /* Darker primary for shadow */
        }
      `}</style>
    </div>
  );
